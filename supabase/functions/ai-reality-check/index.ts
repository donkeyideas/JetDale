...
5. **Scope Creep Risks** — Which features sound simple but are actually complex? What "v2 features" are actually required for a viable v1?
   ...
   **Also flag, anywhere they appear across artifacts** (User Journey, Go-to-Market, Vision, Budget, anywhere):
   ...
   - **Persona pain points not addressed by Architecture or Scope.** For each persona's stated frustration (especially "Resolution requirement:" tags), check whether Architecture Overview or Scope explicitly includes a feature or design decision that solves it. If a persona says "I spend 40% of my week reconciling timestamps" and no architecture component addresses timestamp normalization, that is a high-severity concern and a contradiction between Personas and Architecture.
   - **Success Metric targets that V1 cannot realistically hit.** If Success Metrics sets a target (e.g., "<2% false positive rate") but Scope describes a V1 implementation that cannot reach it (e.g., "ML classifier trained on synthetic data," "rules-based prototype," "manual triage"), flag as a high-severity contradiction. V1 targets must reflect V1 maturity, not steady-state ambition. Mature ML targets do NOT apply to cold-start synthetic-data implementations.
   - **Distributed-system timing assumptions that ignore the deployment environment.** If acceptance criteria specify tight time windows (10-second buffer, sub-minute retry, sub-second sync) but the deployment environment (described in Personas / Vision / Scope) is cellular IoT, hospital networks, field devices, or any partially-connected environment, flag as a high-severity reality concern. Example: "10-second out-of-sequence buffer in a system that ingests from cellular hospital tablets — those tablets can be offline for hours."
   - **Regulated identifying data stored directly in an immutable layer.** If the architecture includes an immutable / append-only / hash-chained / blockchain / ledger layer AND records in that layer contain identifying fields subject to deletion mandates (patient names, MRNs, prescription IDs, nurse IDs, employee IDs, account numbers, emails, etc.), flag as a high-severity legal/compliance concern. A right-to-be-forgotten request or court order cannot be honored on an immutable chain — the architecture must segregate PHI/PII into a separate mutable encrypted store with cryptographic surrogate keys (UUIDs) in the ledger. This is the crypto-shredding pattern; absence of it fails standard hospital IT and GDPR vetting.
   - **State-erasing manual override on audit-trail / compliance / financial events.** If the UX described in Scope, User Journey, or Wireframes proposes "manual reconcile," "manual override," "dismiss anomaly," "mark resolved," "bulk clear," or similar actions that change a flagged audit record's state to cleared/resolved, flag as a high-severity insider-threat concern. The correct pattern is attestation/annotation: users can attach signed notes and supporting documents to a flagged event without altering it. State-erasure of audit/compliance events lets the corrupt actor whose action triggered the alert wipe their own audit signal.
   - **Dashboard payload size incompatible with stated latency target.** If a dashboard / list / index endpoint has a stated latency target (e.g., "2-second load for 500 items on 4G") and the architecture describes nested multi-stream payloads or full data fetched in the list response, calculate the implied payload size against the network class and flag if the math doesn't work. Hospital corporate VPN over 4G ≈ 1-3 Mbps sustained → sub-2s implies <500KB total. The architecture must redesign the list endpoint to return thin summary fields with deep payload deferred to per-item detail endpoints, or the latency target must be adjusted.
