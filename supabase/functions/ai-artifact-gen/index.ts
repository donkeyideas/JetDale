...
const MODEL_MAP: Record<string, 'deepseek-v4-flash'> = {
  demand_analysis: 'deepseek-v4-flash',
...
// demand_analysis runs first — it's the gate that informs every
// downstream artifact (a "weak demand" verdict should anchor budget
// and timeline; a "strong" one should embolden them).
const DEFAULT_ORDER = [
  'demand_analysis',
...
function buildArtifactPrompt(
  ...
  const prompts: Record<string, string> = {
    demand_analysis: `Generate a Demand Analysis for the project "${projectName}".
...
Output the following sections in this exact order using ## headers. Use the markdown table syntax exactly as shown — each row on its own line, no missing pipes.
...
## Dimensions
A markdown table with these exact columns and exactly these six rows. Score each 1-5 using the anchors below. Each row needs a one-sentence rationale and a Source column that is either a discovery answer key (e.g. \`discovery: q12\`), a verifiable public source, or the literal string **estimated, unverified**.
...
- **Cost:** dollar estimate (use the ledger's currency)
- **Time:** hours or days
...
1. If the verdict is WEAK or MODERATE: state that the budget and timeline in the Decision Ledger should be sized for *validation*, not full build, and name the specific cuts.
...
Write 800-1300 words total. No emojis. No fluff. Do not use the words "delve", "leverage", "robust", "cutting-edge", "seamless", "navigate", "harness", "unlock", "game-changer", "paradigm", "synergy", or "holistic".`,
