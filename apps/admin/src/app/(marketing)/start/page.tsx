...
// ============================================================
// Jetdale — Start a Project (Discovery Flow)
// 12 discovery questions → sign up at the end to save.
// All answers stored locally until user creates account.
// ============================================================
...
export default function StartPage() {
  ...
  async function createProjectAndRedirect(userId: string) {
  ...
  async function handleAuth() {
    ...
    try {
      ...
      if (isLogin) {
        ...
        createProjectAndRedirect(data.user.id);
      } else {
  ...
  function loadAnswer(idx: number) {
    ...
    if (next.type === 'open_text') {
      ...
      // Reverse-map labels back to values for the selection UI
      setCurrentText('');
      ...
      <div style={{ minHeight: 'calc(100vh - 200px)', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 32px' }}>
          <div style={{ maxWidth: 440, width: '100%' }}>
            ...
            <h2 style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontSize: 'clamp(28px, 4vw, 40px)', fontWeight: 600, letterSpacing: '-.035em', lineHeight: 1.1, marginBottom: 12 }}>
            ...
            {/* Error */}
            {authError && (
              <p style={{ fontSize: 13, color: 'var(--accent)', marginBottom: 16 }}>{authError}</p>
            )}
            ...
            <button
              ...
              style={{
                ...
                background: authLoading ? 'var(--paper-3)' : 'var(--ink)',
                color: authLoading ? 'var(--muted)' : 'var(--paper)',
                padding: '16px 24px', borderRadius: 999, fontWeight: 600, fontSize: 15, border: 'none',
                cursor: authLoading ? 'default' : 'pointer', transition: 'all .2s',
              }}
            >
              {authLoading
                ? 'Creating account...'
            ...
            <div style={{ textAlign: 'center', marginTop: 16 }}>
              <button
                onClick={() => { setShowSignup(false); setShowDone(true); setAuthError(''); }}
            ...
            <button
              onClick={() => {
                if (isLoggedIn && currentUserId) {
                  createProjectAndRedirect(currentUserId);
                } else {
                  setShowDone(false);
                  setShowSignup(true);
                }
    ...
    <div style={{ minHeight: 'calc(100vh - 200px)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 32px' }}>
        <div style={{ maxWidth: 780, width: '100%' }}>
          ...
          {/* Keyboard hint (only for text inputs) */}
          {q.type === 'open_text' && (
            <div style={{
